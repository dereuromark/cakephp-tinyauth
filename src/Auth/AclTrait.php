...
trait AclTrait {
	...
	protected function _getRolesFromDb(string $rolesTableKey): array {
		try {
			...
			$resultSet = $rolesTable->find()->all();
			if (method_exists($resultSet, 'combineBy')) {
				$result = $resultSet->combineBy($this->getConfig('aliasColumn'), 'id');
			} else {
				// @codeCoverageIgnoreStart
				$result = $resultSet->combine($this->getConfig('aliasColumn'), 'id');
